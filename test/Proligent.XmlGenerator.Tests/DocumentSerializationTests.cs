...
public class DocumentSerializationTests
{
    ...
    public void SequenceRun_Includes_Document_Elements_When_Populated()
    {
        ...
        var seqElement = opElement
            .Elements(XmlNamespaces.Dw + "SequenceRun")
            .First(e =>
                e.Attribute("SequenceFullName")?.Value == "Seq"
                || e.Attribute("SequenceRunId") != null
            );
    ...
    {
        var opDoc = new Document(
            fileName: "operation-file.txt",
            identifier: "op-id",
            name: "OpDoc"
        );
        var operation = new OperationRun(
    ...
    {
        var puDoc = new Document(
            fileName: "product-file.txt",
            identifier: "pu-id",
            description: "PU Desc"
        );
        var product = new ProductUnit(
            productUnitIdentifier: "PU1",
            productFullName: "Product/One",
            documents: new[] { puDoc }
        );
